...
function drawHealthRank() {
  ...
  const full = pool.filter(x => x.h.na === 0).sort((a, b) => b.h.tot - a.h.tot);   // 只排滿格，總分高→低
  const incomplete = pool.length - full.length;
  ...
  note.innerHTML = `${isEtf ? "ETF" : "個股"}共 <b>${full.length}</b> 檔滿格參與排名`
    + (incomplete ? `；另 ${incomplete} 檔資料不全（缺格未列入，避免缺分母灌分）` : "")
    + `。名次＝依總分高→低；分數為 12 指標對照固定門檻加權，<b>非買賣訊號、未回測</b>。`;
}
