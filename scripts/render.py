...
:root {
  /* shadcn zinc 深色：中性近黑，靠細邊框而非重陰影 */
  --bg: #09090b; --panel: #121214; --panel2: #18181b; --border: #27272a; --border-hi: #3f3f46;
  --fg: #fafafa; --muted: #a1a1aa; --head: #f4f4f5;
...
/* 體檢榜（四燈號可排序總表 + 分布）；體檢非選股、未回測 */
.hr-ctrl { display: flex; flex-wrap: wrap; align-items: center; gap: 12px; margin: 12px 0 8px; }
.hr-cohort { display: flex; gap: 6px; }
.hr-cohort .chip { padding: 5px 12px; border: 1px solid var(--border); border-radius: 999px; background: var(--surface); color: var(--muted); font-size: .82rem; cursor: pointer; transition: border-color var(--tr), color var(--tr); }
.hr-cohort .chip.active { border-color: var(--accent); color: var(--accent); font-weight: 600; }
.hr-dist { display: flex; gap: 6px; flex-wrap: wrap; font-size: .78rem; font-family: var(--num); }
.hr-dist span { padding: 3px 10px; border-radius: 999px; border: 1px solid var(--border); }
.hr-dist .s { color: var(--hc-strong); background: var(--hc-strong-s); }
.hr-dist .m { color: var(--hc-mid); background: var(--hc-mid-s); }
.hr-dist .w { color: var(--hc-weak); background: var(--hc-weak-s); }
.hr-wrap { overflow-x: auto; }
.hr-tbl { width: 100%; border-collapse: collapse; font-size: .84rem; }
.hr-tbl th { position: sticky; top: 0; z-index: 1; background: var(--surface); cursor: pointer; white-space: nowrap; text-align: right; padding: 7px 9px; border-bottom: 1px solid var(--border); color: var(--muted); font-weight: 600; user-select: none; }
.hr-tbl th:nth-child(2) { text-align: left; }
.hr-tbl td { text-align: right; padding: 6px 9px; border-bottom: 1px solid var(--border); font-family: var(--num); font-variant-numeric: tabular-nums; white-space: nowrap; }
.hr-tbl td:nth-child(2) { text-align: left; font-family: inherit; cursor: pointer; }
.hr-tbl tbody tr:hover td { background: var(--accent-soft); }
.hr-tot { font-weight: 800; }
.hr-tot.s { color: var(--hc-strong); } .hr-tot.m { color: var(--hc-mid); } .hr-tot.w { color: var(--hc-weak); }
.hr-face.s { color: var(--hc-strong); } .hr-face.m { color: var(--hc-mid); } .hr-face.w { color: var(--hc-weak); }
.hr-face.n { color: var(--muted); }
...
.scr-chip:hover { border-color: var(--border-hi); color: var(--fg); }
.scr-chip.on { background: var(--accent-soft); border-color: var(--accent); color: var(--accent); font-weight: 600; }
.scr-count { font-size: .82rem; color: var(--muted); margin: 4px 0 8px; }
.scr-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(215px, 1fr)); gap: 7px; }
...
/* 使用說明 / 新手導覽 */
.help-body { font-size: .9rem; line-height: 1.7; }
.help-body p { margin: 10px 0; }
.help-tabs { display: grid; gap: 7px; margin: 10px 0; }
.help-tabs > div { border: 1px solid var(--border); border-radius: 8px; padding: 8px 11px; font-size: .84rem; color: var(--muted); }
.help-tabs b { color: var(--fg); margin-right: 6px; }
...
:root[data-theme="light"] {
  color-scheme: light;
  /* shadcn zinc 淺色：白卡 + 極淡中性邊框、柔陰影 */
  --bg: #f7f7f8; --panel: #ffffff; --panel2: #fafafa; --border: #e4e4e7; --border-hi: #d4d4d8;
  --fg: #09090b; --muted: #71717a; --head: #18181b;
  --up: #d70015; --down: #16a34a; --flat: #71717a; /* 台股紅漲綠跌（白底可讀） */
...
<header class="top"><div class="top-inner">
  <div class="brand"><svg class="logo" viewBox="0 0 20 20" aria-hidden="true"><rect x="0" y="0" width="12" height="9" rx="1.5" fill="#e0433f"/><rect x="13" y="0" width="7" height="9" rx="1.5" fill="#00a37a"/><rect x="0" y="10" width="7" height="10" rx="1.5" fill="#00a37a"/><rect x="8" y="10" width="12" height="10" rx="1.5" fill="#e0433f"/></svg><h1>台股產業地圖</h1><span class="sub">自用・現況呈現・不預測</span>
    <div class="searchwrap"><input id="search" placeholder="搜代號/股名…" autocomplete="off"><div id="search-res"></div></div><button class="theme-btn" id="helpBtn" title="使用說明" aria-label="使用說明" onclick="showHelp()">?</button><button class="theme-btn" id="themeBtn" title="切換深/淺色" aria-label="切換深淺色"></button></div>
  <nav class="tabs" id="tabs">
    ...
    <button class="tab" data-pane="health">體檢榜</button>
...
<section id="sec-chains"><h2>產業價值鏈 <span class="stamp" data-stamp="chains_view"></span></h2><div class="sub">內容自產（上中下游整理，非官方分類、非投資建議）｜點個股開 Yahoo 股市頁</div><div id="chain-chips"></div><div id="chains"></div></section>
</div>
...
<section id="sec-screener"><h2>多條件選股器</h2>
<div class="sub">勾選條件取「交集」（AND）｜法人連買賣、技術狀態、量價全用寫死規則｜結果為現況篩選、非買賣建議</div>
<div id="scr-conds"></div>
<div id="scr-result"></div></section>
<section id="sec-margintrend"><h2>大盤融資餘額趨勢 <span class="stamp" data-stamp="margintrend"></span></h2>
<div class="sub">近期全市場融資餘額（億）｜升＝散戶槓桿加碼、降＝去槓桿｜現況描述、非訊號</div>
<div id="margintrend"></div></section>
...
<div class="tabpane" id="pane-health">
<section id="sec-healthrank"><h2>體檢榜 <span class="stamp" data-stamp="health"></span></h2>
<div class="sub"><b>體檢非選股</b>：把 12 個現有指標對照固定門檻加權成分數（基本/籌碼/技術 40/30/30、ETF 走 2 面制），<b>未回測、不保證預測力</b>，只當「現況一眼看完」。<b>紅＝強、綠＝弱</b>（台股語意）。個股（滿分 100）與 ETF（滿分 50）<b>分開排、不同表</b>；缺格股不列入避免灌分。點欄位標題排序、點列開個股。</div>
<div class="hr-ctrl">
  <div class="hr-cohort"><button class="chip active" data-cohort="stock">個股 · 滿分100</button><button class="chip" data-cohort="etf">ETF · 滿分50</button></div>
  <div id="hr-dist" class="hr-dist"></div>
</div>
<div id="hr-table" class="hr-wrap"></div>
<div id="hr-note" class="sub" style="margin-top:8px"></div></section>
</div>
...
// ── 新手導覽 / 使用說明（複用 overlay；首訪自動彈一次）──
function showHelp() {
  const panel = document.getElementById("sp-panel");
  panel.classList.remove("sp-wide");
  panel.innerHTML = `<button class="sp-close" onclick="spClose()">✕</button>
    <h3>使用說明 <span class="sub">30 秒看懂</span></h3>
    <div class="help-body">
      <p><b>這是什麼？</b>台股盤後數據地圖。只呈現「現況」，<b>不預測、不報明牌</b>。所有數字每交易日收盤後自動更新、規則寫死可重現。</p>
      <div class="help-tabs">
        <div><b>每日焦點</b>今日一句摘要、異常警示、三大法人、外資趨勢、大盤情緒曲線。</div>
        <div><b>市場熱力</b>產業熱力圖、強弱排行、逼近 52 週高／低。</div>
        <div><b>觀測站</b>多條件選股器、個股比較、大戶動向、名詞小百科。</div>
        <div><b>題材／產業鏈</b>題材分類、產業上下游關係。</div>
      </div>
      <p><b>怎麼操作？</b>右上搜代號 → 開個股面板；點 <b>★</b> 加自選、<b>🏷</b> 分組；右上 <b>🌙</b> 切深色。</p>
      <p class="sub">看不懂名詞？到「觀測站 → 名詞小百科」查白話解釋。所有清單皆為現況描述、非買賣建議。</p>
    </div>`;
  document.getElementById("sp-overlay").style.display = "block";
  panel.style.display = "block";
}
window.showHelp = showHelp;
try { if (!localStorage.getItem("twmm_seen_help")) { showHelp(); localStorage.setItem("twmm_seen_help", "1"); } } catch (e) {}
...
// ── 體檢榜（四燈號健診可排序總表 + 分布）──
// 定位：不是選股訊號、不做編輯精選 Top-N。這是「可排序體檢表」——你自己點欄位排，
// 性質是查詢工具。個股(滿分100)/ETF(滿分50) 分開排（混排=蘋果比橘子），缺格股不列入
// （避免缺分母灌分，呼應當初 0050 湊 90 分的坑）。分數未回測、無預測力保證。
let HR_ALL = [], HR_COHORT = "stock", HR_BUILT = false;
function ensureHealthRank() {
  if (HR_BUILT) return;
  HR_BUILT = true;
  const wrap = document.getElementById("hr-table");
  wrap.innerHTML = '<div class="sub">載入健診資料…</div>';
  loadHealth().then(H => {
    const st = H && H.data && H.data.stocks;
    const stamp = document.querySelector('[data-stamp="health"]');
    if (stamp) stamp.innerHTML = H ? stampFor(H) : "";
    if (!st) { wrap.innerHTML = '<div class="err">健診資料抓取失敗</div>'; return; }
    HR_ALL = Object.keys(st).filter(c => QUOTES[c]).map(c => ({ c, h: st[c], q: QUOTES[c] }));
    drawHealthRank();
  });
}
function drawHealthRank() {
  const wrap = document.getElementById("hr-table"), dist = document.getElementById("hr-dist"),
        note = document.getElementById("hr-note");
  const isEtf = HR_COHORT === "etf";
  const MIN_VAL = 1;   // 個股成交值地板（億）：低流動雞蛋水餃股籌碼/技術全是噪音，幾張就灌綠 → 剔除
  const pool = HR_ALL.filter(x => (x.h.kind === "etf") === isEtf);
  const scored = pool.filter(x => x.h.na === 0);                       // 滿格
  const thin = isEtf ? [] : scored.filter(x => (x.q[6] || 0) < MIN_VAL);  // 滿格但流動性不足
  const full = (isEtf ? scored : scored.filter(x => (x.q[6] || 0) >= MIN_VAL))
    .sort((a, b) => b.h.tot - a.h.tot);   // 只排滿格且夠流動，總分高→低
  const incomplete = pool.length - scored.length;
  // C 分布（強≥70% / 中40-69% / 弱<40% of max）
  const band = { s: 0, m: 0, w: 0 };
  full.forEach(x => band[hcTotTone(x.h.tot, x.h.max)]++);
  dist.innerHTML = `<span class="s">強 ${band.s}</span><span class="m">中 ${band.m}</span><span class="w">弱 ${band.w}</span>`;
  // D 表
  const FN = ["基本", "籌碼", "技術", "估值"];
  const faceCols = isEtf ? [1, 2] : [0, 1, 2, 3];   // ETF 只有籌碼/技術
  const head = `<tr><th>名次</th><th>股票</th><th>總分</th>`
    + FN.map((n, i) => faceCols.includes(i) ? `<th>${n}</th>` : "").join("")
    + `<th>成交值</th><th>現價</th><th>漲跌</th></tr>`;
  const body = full.map((x, i) => {
    const h = x.h, q = x.q, tone = hcTotTone(h.tot, h.max);
    const faceCells = FN.map((n, fi) => {
      if (!faceCols.includes(fi)) return "";
      const sc = (h.sc || [])[fi];
      return sc == null ? `<td class="hr-face n">–</td>` : `<td class="hr-face ${hcFaceTone(sc)}">${sc}</td>`;
    }).join("");
    return `<tr onclick="if(window.spClose)spClose();openStock('${x.c}')">`
      + `<td>${i + 1}</td>`
      + `<td>${q[1]} <span class="sub">${x.c}</span></td>`
      + `<td class="hr-tot ${tone}">${h.tot}<span class="sub">/${h.max}</span></td>`
      + faceCells
      + `<td>${(q[6] || 0).toLocaleString()}<span class="sub">億</span></td>`
      + `<td class="${cls(q[4])}">${q[3]}</td>`
      + `<td class="${cls(q[4])}">${sign(q[4])}%</td></tr>`;
  }).join("");
  wrap.innerHTML = `<table class="hr-tbl"><thead>${head}</thead><tbody>${body}</tbody></table>`;
  note.innerHTML = `${isEtf ? "ETF" : "個股"}共 <b>${full.length}</b> 檔參與排名`
    + (isEtf ? "" : `（成交值 ≥ ${MIN_VAL} 億；濾掉 ${thin.length} 檔低流動雞蛋水餃股——它們幾張成交就把籌碼/技術面灌綠，是噪音不是強）`)
    + (incomplete ? `；另 ${incomplete} 檔資料不全（缺格未列入，避免缺分母灌分）` : "")
    + `。名次＝依總分高→低。<b>提醒：分數量的是「動能＋當下快照」不是公司品質</b>——正在被拉的股票技術/籌碼面會滿分。非買賣訊號、未回測。`;
}
document.querySelectorAll(".hr-cohort .chip").forEach(b =>
  b.addEventListener("click", () => {
    if (HR_COHORT === b.dataset.cohort) return;
    HR_COHORT = b.dataset.cohort;
    document.querySelectorAll(".hr-cohort .chip").forEach(x => x.classList.toggle("active", x === b));
    drawHealthRank();
  }));
  ...
  function show(id, push) {
    ...
    el.innerHTML = `<div class="topic-desc">${ch.desc}</div>` + ch.stages.map(st => `
      ...
      st.nodes.map(nd => `<div class="node"><div class="node-label">${nd.label} <span class="sub">${nd.members.length} 檔</span></div>
        ...
        nd.members.map(m => `<div class="co" data-code="${m.code}" data-mkt="${m.market || "twse"}" title="開 Yahoo 股市 ${m.code}${fundLine(m.code) ? "&#10;" + fundLine(m.code) : ""}">
...
(function () {   // ── 大盤融資餘額趨勢（讀 market_trend，折線，跳過缺值）──
  const env = DATA.market_trend, el = document.getElementById("margintrend");
  const stamp = document.querySelector('[data-stamp="margintrend"]');
  if (stamp) stamp.innerHTML = stampFor(env);
  if (!el) return;
  if (!env || !env.ok) { el.innerHTML = `<div class="err">融資趨勢資料失敗：${(env && env.error) || ""}</div>`; return; }
  const s = (env.data.series || []).slice(-20).filter(d => d.margin_bal != null);
  if (s.length < 2) { el.innerHTML = `<div class="sub">融資餘額歷史不足</div>`; return; }
  const W = 720, H = 156, padT = 16, padB = 22, padL = 46, padR = 8;
  const plotH = H - padT - padB, plotW = W - padL - padR, n = s.length;
  const vals = s.map(d => d.margin_bal);
  let lo = Math.min(...vals), hi = Math.max(...vals); const pad = (hi - lo) * 0.15 || 1; lo -= pad; hi += pad;
  const X = i => padL + (n === 1 ? plotW / 2 : i * plotW / (n - 1));
  const Y = v => padT + (hi - v) / (hi - lo) * plotH;
  const pts = s.map((d, i) => `${X(i).toFixed(1)},${Y(d.margin_bal).toFixed(1)}`).join(" ");
  const dots = s.map((d, i) => `<circle cx="${X(i).toFixed(1)}" cy="${Y(d.margin_bal).toFixed(1)}" r="2.6" fill="var(--accent)"><title>${d.date}　融資餘額 ${d.margin_bal} 億</title></circle>`).join("");
  const labels = [0, Math.floor(n / 2), n - 1].map(i => `<text x="${X(i).toFixed(1)}" y="${H - 7}" text-anchor="middle">${s[i].date.slice(5)}</text>`).join("");
  const last = s[s.length - 1], chg = last.margin_bal - s[0].margin_bal;
  el.innerHTML = `<div class="trend-head">
      <span>最新融資餘額 <b>${last.margin_bal.toLocaleString()}</b> 億</span>
      <span class="sub">近 ${n} 日 <b class="${chg >= 0 ? "up" : "down"}">${chg > 0 ? "+" : ""}${chg.toFixed(0)}</b> 億</span>
    </div>
    <svg class="sent-svg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" role="img">
...
(function () {   // ── 多條件選股器（法人 QUOTES + 技術 ta，取交集）──
  const condsEl = document.getElementById("scr-conds");
  const resEl = document.getElementById("scr-result");
  if (!condsEl || !resEl) return;
  const CONDS = [
    { k: "fbuy3", label: "外資連買≥3", tech: false, test: q => (q[9] || 0) >= 3 },
    { k: "tbuy3", label: "投信連買≥3", tech: false, test: q => (q[10] || 0) >= 3 },
    { k: "cobuy", label: "外資投信同買", tech: false, test: q => (q[7] || 0) > 0 && (q[8] || 0) > 0 },
    { k: "up", label: "今日上漲", tech: false, test: q => (q[4] || 0) > 0 },
    { k: "liq", label: "成交值≥1億", tech: false, test: q => (q[6] || 0) >= 1 },
    { k: "above_year", label: "站上年線", tech: true, test: (q, t) => t && t.above && t.above["240"] === true },
    { k: "ma_bull", label: "多頭排列", tech: true, test: (q, t) => t && (t.signals || []).includes("ma_bull") },
    { k: "vol_spike", label: "今日爆量", tech: true, test: (q, t) => t && (t.signals || []).includes("vol_spike") },
    { k: "near_high", label: "逼近52週高", tech: true, test: (q, t) => t && t.pos52w != null && t.pos52w >= 0.9 },
  ];
  const active = new Set();
  condsEl.innerHTML = `<div class="scr-chips">${CONDS.map(c =>
    `<button class="scr-chip" data-k="${c.k}">${c.label}</button>`).join("")}</div>`;

  function compute(taData) {
    const st = (taData && taData.data && taData.data.stocks) || {};
    const picks = CONDS.filter(c => active.has(c.k));
    let rows = (DATA.search || []).filter(q =>
      picks.every(c => c.test(q, st[q[0]]))).sort((a, b) => (b[6] || 0) - (a[6] || 0));
    const total = rows.length;
    rows = rows.slice(0, 100);
    const names = picks.map(c => c.label).join(" ＋ ");
    if (!rows.length) { resEl.innerHTML = `<div class="scr-count">符合「${names}」：0 檔</div>`; return; }
    const cards = rows.map(q => {
      const fs = q[9] ? `<span class="sub">外連${q[9]}</span> ` : "";
      const ts = q[10] ? `<span class="sub">投連${q[10]}</span>` : "";
      return `<div class="scr-card" onclick="openStock('${q[0]}')">
        <b>${q[1]}</b> <span class="sub">${q[0]}${q[2] ? "・" + q[2] : ""}</span><br>
        <span class="${cls(q[4])}">${sign(q[4])}%</span> <span class="sub">${q[3]}｜${q[6]}億</span> ${fs}${ts}</div>`;
    }).join("");
    resEl.innerHTML = `<div class="scr-count">符合「${names}」：${total} 檔${total > 100 ? "（顯示成交值前 100）" : ""}</div>
      <div class="scr-grid">${cards}</div>`;
  }
  function refresh() {
    if (!active.size) { resEl.innerHTML = `<div class="scr-count">勾選上方條件開始篩選（可多選、取交集）。</div>`; return; }
    if (CONDS.some(c => c.tech && active.has(c.k))) {
      resEl.innerHTML = `<div class="scr-count">載入技術面…</div>`;
      loadTa().then(compute);
    } else compute(null);
  }
  condsEl.querySelectorAll(".scr-chip").forEach(btn => btn.addEventListener("click", () => {
    const k = btn.dataset.k;
    if (active.has(k)) { active.delete(k); btn.classList.remove("on"); }
    else { active.add(k); btn.classList.add("on"); }
    refresh();
  }));
  refresh();
})();
...
// ── 分頁切換 ──
(function () {
  const PANES = ["focus", "radar", "topics", "chains", "market", "watch", "fund", "health", "news"];
  function show(id, push) {
    if (!PANES.includes(id)) id = "focus";
    if (id === "health") ensureHealthRank();   // 首次開才 lazy fetch health.json
